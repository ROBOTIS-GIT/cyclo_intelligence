...
const InferencePanel = () => {
  ...
  const showPolicyPath = !isRldx || rldxRuntimeMode !== 'client';
  ...
  const handleRldxRuntimeModeChange = useCallback((mode) => {
    if (!isEditable || !['client', 'local', 'server'].includes(mode)) return;
    const updates = { rldxRuntimeMode: mode };
    if (mode === 'local') {
      updates.remoteHost = '127.0.0.1';
      updates.remotePort = Number(info.remotePort || 5555);
    }
    dispatch(setInferenceTaskInfo(updates));
    dispatch(markLocalTaskInfoEdited({ source: 'inference' }));
  ...
  return (
    <div className={classInfoPanel}>
          ...
          <div className="flex items-center justify-between gap-2 mb-1.5">
            ...
            <span className="text-xs font-semibold text-gray-600">
              {rldxRuntimeMode === 'local'
                ? 'Local Stack'
                : rldxRuntimeMode === 'server' ? 'Server' : 'Client'}
            </span>
          </div>
          <div className="grid grid-cols-3 gap-1">
            {[
              ...
              <button
                ...
                disabled={!isEditable}
                className={actionModeButtonClass(rldxRuntimeMode === mode)}
              >
                {label}
              </button>
            ))}
      ...
      {/* Policy Path */}
      {showPolicyPath && <div className={clsx('flex', 'items-start', 'mb-2.5')}>
