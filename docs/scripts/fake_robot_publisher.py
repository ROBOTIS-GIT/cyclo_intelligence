...
Publishes the 4 RGB cameras + /joint_states + /odom at 10 Hz with
black images and zero joint positions. Just enough to fill the
RobotClient buffer so Process A can build an observation batch when
Process B publishes a run_inference trigger.
"""
import sys
import numpy as np
import cv2
