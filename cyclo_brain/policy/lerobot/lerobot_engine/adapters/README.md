...
| `input_plan_factory` | Declare required sources, temporal offsets and tensor transforms |
...
`input_plan_factory` receives camera mapping, state modalities, image/state
transforms, `policy_config`, `model_path` and the default freshness limit. It
returns an `InputAssembler`. It runs during LOAD, not on every prediction.
...
Use `ExecutionQuery`, not time-offset sensor queries, for execution facts:

```python
InputField("previous_commands", (
    ExecutionQuery("execution:published:command", count=2, min_count=0),
))
InputField("next_commands", (
...
Protocol 3.0 separates the 64 KiB compression threshold from the 8 MiB wire and
