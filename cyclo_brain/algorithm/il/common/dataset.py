"""Model-neutral LeRobot v3 data-epoch adapter for imitation learning.
...
class RootSelection:
    ...
    @property
    def selected_episodes(self) -> tuple[int, ...]:
        """Selected root-local episodes, independent of outcome-label policy."""

        return self.success_episodes
...
class VirtualLeRobotDataset(torch.utils.data.Dataset):
    """Expose compatible selected roots as one logical LeRobot dataset."""
...
def load_virtual_lerobot_dataset(
    selections: Sequence[RootSelection],
    *,
    policy_config: Any,
    ...
    """Load selected demonstrations and construct a virtual LeRobot dataset.
